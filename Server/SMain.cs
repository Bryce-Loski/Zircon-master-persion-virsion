...
namespace Server
{
    public partial class SMain : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void UpdateInterface()
        {
            ...
            ObjectLabel.Caption = string.Format(@"对象: {0:#,##0}/{1:#,##0}", SEnvir.ActiveObjects.Count, SEnvir.Objects.Count);
            ProcessLabel.Caption = string.Format(@"处理: {0:#,##0}", SEnvir.ProcessObjectCount);
            LoopLabel.Caption = string.Format(@"循环: {0:#,##0}", SEnvir.LoopCount);
            EMailsSentLabel.Caption = string.Format(@"邮件: {0:#,##0}", EmailService.EMailsSent);

            ConDelay.Caption = string.Format(@"连接延迟: {0:#,##0}ms", SEnvir.ConDelay);
            SaveDelay.Caption = string.Format(@"存档延迟: {0:#,##0}ms", SEnvir.SaveDelay);
            ...
            if (SEnvir.DownloadSpeed > GB)
                DownloadSpeedLabel.Caption = string.Format(@"下速: {0:#,##0.0}GBps", SEnvir.DownloadSpeed / GB);
            else if (SEnvir.DownloadSpeed > MB)
                DownloadSpeedLabel.Caption = string.Format(@"下速: {0:#,##0.0}MBps", SEnvir.DownloadSpeed / MB);
            else if (SEnvir.DownloadSpeed > KB)
                DownloadSpeedLabel.Caption = string.Format(@"下速: {0:#,##0}KBps", SEnvir.DownloadSpeed / KB);
            else
                DownloadSpeedLabel.Caption = string.Format(@"下速: {0:#,##0}Bps", SEnvir.DownloadSpeed);

            // 上传速度
            if (SEnvir.UploadSpeed > GB)
                UploadSpeedLabel.Caption = string.Format(@"上速: {0:#,##0.0}GBps", SEnvir.UploadSpeed / GB);
            else if (SEnvir.UploadSpeed > MB)
                UploadSpeedLabel.Caption = string.Format(@"上速: {0:#,##0.0}MBps", SEnvir.UploadSpeed / MB);
            else if (SEnvir.UploadSpeed > KB)
                UploadSpeedLabel.Caption = string.Format(@"上速: {0:#,##0}KBps", SEnvir.UploadSpeed / KB);
            else
                UploadSpeedLabel.Caption = string.Format(@"上速: {0:#,##0}Bps", SEnvir.UploadSpeed);
        }
