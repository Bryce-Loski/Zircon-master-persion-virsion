...
using System;
using Server.Helpers; // 引入 EnumHelper
...
namespace Server.Views
{
    public partial class MonsterInfoView : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        public MonsterInfoView()
        {
            ...
            // === 让 Flag 列使用“显示中文”的枚举下拉框 ===
            var flagCombo = new RepositoryItemImageComboBox();
            flagCombo.AutoHeight = false;
            //flagCombo.Buttons.Add(new EditorButton(ButtonPredefines.Combo));

            foreach (MonsterFlag flag in Enum.GetValues(typeof(MonsterFlag)))
            {
                flagCombo.Items.Add(new ImageComboBoxItem(EnumHelper.GetDescription(flag), flag));
            }

            MonsterInfoGridControl.RepositoryItems.Add(flagCombo);

            gridColumn5.ColumnEdit = flagCombo;
