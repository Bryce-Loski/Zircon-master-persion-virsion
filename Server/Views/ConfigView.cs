...
namespace Server.Views
{
    public partial class ConfigView : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void InitLabelAlign()
        {
            // left = 20 你可以根据需要改，比如 30、40，看界面舒服即可
            const int left = 20;
            const int width = 90;

            //网络tab
            AlignLabelLeft(labelControl1, left, width); // IP 地址
            AlignLabelLeft(labelControl2, left, width); // 端口
            AlignLabelLeft(labelControl3, left, width); // 连接超时
            AlignLabelLeft(labelControl6, left, width); // Ping 间隔
            AlignLabelLeft(labelControl51, left, width); // 用户统计端口
            AlignLabelLeft(labelControl87, left, width); // 最大封包数
            AlignLabelLeft(labelControl86, left, width); // 封包封禁时长

            //邮件tab
            AlignLabelLeft(labelControl26, left, width); //服务器
            AlignLabelLeft(labelControl25, left, width); //端口
            AlignLabelLeft(labelControl27, left, width);//启用SSL
            AlignLabelLeft(labelControl28, left, width);//账号
            AlignLabelLeft(labelControl29, left, width);//密码
            AlignLabelLeft(labelControl30, left, width);//发件人
            AlignLabelLeft(labelControl31, left, width);//显示名称
        }

        private void AlignLabelLeft(LabelControl lbl, int left, int width)
        {
            if (lbl == null) return;
            ...
            // 统一左边坐标和宽度，高度保持不变
            lbl.Location = new Point(left, lbl.Location.Y);
            lbl.Size = new Size(width, lbl.Size.Height);
